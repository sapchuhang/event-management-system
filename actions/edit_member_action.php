...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $stmt = $pdo->prepare("UPDATE members SET member_no = ?, full_name = ?, gender = ?, contact = ?, page_number = ?, table_no = ?, file_number = ?, status = ?, address = ? WHERE id = ?");
        $stmt->execute([$member_no, $full_name, $gender, $contact, $page_number, $table_no, $file_number, $status, $address, $id]);
