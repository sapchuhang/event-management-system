...
if (!isset($_GET['id']) || !is_numeric($_GET['id'])) {
    setFlashMessage('error', 'Invalid user ID parameter.');
...
$assignedTables = getAssignedTables($id);

// Merge any assigned tables that might not be in the members records anymore so they don't get lost
foreach ($assignedTables as $tbl) {
        ...
        <div class="glass-panel p-4 p-md-5">
            <form action="../actions/edit_user_action.php" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="username" class="form-control" required value="<?= htmlspecialchars($user['username']) ?>" placeholder="Enter username" maxlength="50" autocomplete="username">
                    <div class="form-text text-muted">Must be unique, alphanumeric and under 50 characters.</div>
                </div>
                
                <div class="mb-3">
                    <label class="form-label fw-medium">Password</label>
                    <input type="password" name="password" class="form-control" placeholder="Leave empty to keep current password" autocomplete="new-password">
                    <div class="form-text text-muted">Only input a value if you wish to reset/change the user's password. Minimum 6 characters.</div>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <?php if ($isSelf): ?>
                        <select class="form-select" disabled>
                            <option value="admin" selected>Administrator (You cannot change your own role)</option>
                        </select>
                    ...
                    <div class="row g-2 border rounded p-3 bg-light" style="max-height: 200px; overflow-y: auto;">
                        ...
                        <?php else: ?>
                            <?php foreach ($allTables as $table): 
                                $checked = in_array($table, $assignedTables) ? 'checked' : '';
                            ?>
                                <div class="col-6 col-sm-4">
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" name="assigned_tables[]" value="<?= htmlspecialchars($table) ?>" id="table_<?= htmlspecialchars(preg_replace('/[^a-zA-Z0-9_-]/', '_', $table)) ?>" <?= $checked ?>>
                ...
                <button type="submit" class="btn btn-primary-custom w-100"><i class="fas fa-save me-2"></i> Update User Account</button>
            </form>
        </div>
    </div>
</div>
